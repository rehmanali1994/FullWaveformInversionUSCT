function wf_x_z_f = upward_continuation(x, z, c_x_z, f, tx_x_f, aawin)
...
for f_idx = 1:numel(f)
    ...
    wf_x_z_f(numel(z), :, f_idx) = tx_x_f(:,f_idx); % Injection Surface (z = 0)
    for z_idx = numel(z):-1:2
        ...
        wf_x_z_f(z_idx-1, :, f_idx) = ...
            ift(H.*ft(dH.*squeeze(wf_x_z_f(z_idx, :, f_idx)))); 
    end
end
